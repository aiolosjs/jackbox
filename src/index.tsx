...
const handleImport = (file: string) => {
  ...
  const finalContent = (file || '').replace(importRegex, matchedImport => {
    if (matchedImport.includes('.')) {
      ...
      if (matchedImport.includes(',')) {
        const replaceLib = matchedImport.replace(/['|"].*['|"]/, "'@aiolosjs/components'");
      ...
      // relative import;
      if (matchedImport.includes('{')) {
        return matchedImport.replace(/['|"].*['|"]/, "'@aiolosjs/components'");
      }
      return matchedImport
        .replace('import ', 'import { ')
        .replace(' from', ' } from')
        .replace(/['|"].*['|"]/, "'@aiolosjs/components'");
    }
    // absolute import
    const pkgNameRegex = /['|"](.*)['|"]/;
